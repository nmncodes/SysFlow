...
export default function CompareModal({ node, nodes, edges, targetRps, failures, onApply, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/20 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-semibold text-zinc-900">Compare alternative</h3>
        ...
        <label className="mt-4 block text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
          ...
          <select
            ...
            className="mt-1.5 w-full rounded-lg border border-zinc-200 bg-white px-2.5 py-2 text-sm text-zinc-700 outline-none focus:border-violet-400"
          >
            {alternatives.map((c) => <option key={c.type} value={c.type}>{c.label}</option>)}
          </select>
        </label>

        <button onClick={runCompare} disabled={isComparing} className="mt-4 w-full rounded-lg bg-zinc-900 px-3 py-2.5 text-sm font-semibold text-white hover:bg-zinc-700 disabled:opacity-50">
        ...
        {current && alternative && (
          <div className="mt-5">
            <div className="grid grid-cols-3 gap-2 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">
              <span>Metric</span>
              <span className="text-center">{labelFor(current.type)}</span>
              <span className="text-center text-violet-600">{labelFor(alternative.type)}</span>
            </div>
            <div className="mt-2 divide-y divide-zinc-100 rounded-xl border border-zinc-100">
              {METRIC_ROWS.map((row) => {
                ...
                return (
                  <div key={row.label} className="grid grid-cols-3 items-center gap-2 px-3 py-2 text-sm">
                    ...
                    <span className="text-center text-zinc-700">{Math.round(a * 100) / 100}{row.unit}</span>
                    <span className={`text-center font-semibold ${better ? 'text-emerald-600' : worse ? 'text-red-500' : 'text-zinc-700'}`}>
              ...
              <div className="grid grid-cols-3 items-center gap-2 px-3 py-2 text-sm">
                <span className="text-xs text-zinc-500">Est. monthly cost</span>
                <span className="text-center text-zinc-700">${current.monthlyCost.toLocaleString()}</span>
                <span className={`text-center font-semibold ${alternative.monthlyCost < current.monthlyCost ? 'text-emerald-600' : alternative.monthlyCost > current.monthlyCost ? 'text-red-500' : 'text-zinc-700'}`}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={onClose} className="rounded-xl px-3 py-2 text-sm text-zinc-500 hover:bg-zinc-50">Cancel</button>
